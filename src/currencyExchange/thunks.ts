    ...
    return async (dispatch: AppDispatch) => {
        try {
            const result = await fetch(
                `https://free.currconv.com/api/v7/convert?q=SEK_${currency}&compact=ultra&apiKey=17a09f2fac4d24c4f5ee`
            );
            const currencyInfo = await result.json();
            dispatch(
                CurrencyExchangeAction.currencyExchangeRequest(
                    formatCurrencyInfo(currencyInfo)
                )
            );
        } catch (e) {
            alert(e);
